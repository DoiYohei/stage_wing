...
describe("setLineupForShow", () => {
  it("returns lineup for show", () => {
    const event = {
      ...
      unregistered_performers: ["band3", "band4"],
    };
    const lineup = setLineupForShow(event);
    expect(lineup).toEqual([
      { text: "band1", to: "/bands/1" },
      { text: "band2", to: "/bands/2" },
      { text: "band3" },
      { text: "band4" },
    ]);
  });
...
describe("patchLineup", () => {
  it("calls postLineup twice and deleteLineup once", async () => {
    ...
    expect(axios.delete.mock.calls[0][1].data.get("lineup[performer_id]")).toBe(
      "3"
    );
  });
});
